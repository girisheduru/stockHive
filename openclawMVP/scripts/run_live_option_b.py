...
def load_tickers() -> list[str]:
    payload = json.loads((AGENT_SYSTEM / "config" / "nasdaq100-tickers.json").read_text())
...
def run_decision_engine(payload: dict[str, Any]) -> dict[str, Any]:
    proc = subprocess.run(
        [sys.executable, str(CORE_SCRIPTS / "decision_engine.py")],
...
def format_message(run_date: str, decision: dict[str, Any], mode: str) -> str:
    ...
    if top5:
        ...
        lines.append("No buy candidates passed the current decision rules.")
        if excluded:
            lines.append(f"All reviewed candidates were excluded ({len(excluded)} total).")
            reason_counts: dict[str, int] = {}
            for row in excluded:
                reason = row.get("reason", "unspecified")
                key = reason.split()[0] if reason else "unspecified"
                reason_counts[key] = reason_counts.get(key, 0) + 1
            lines.append("Summary: " + ", ".join(f"{k}={v}" for k, v in sorted(reason_counts.items())))
    if excluded:
...
def main() -> int:
    ...
    (LIVE_OUTPUT / "merged.json").write_text(json.dumps(merged, indent=2) + "\n")

    log("[STAGE 4/6]", "Running decision engine.")
    decision = run_decision_engine(merged)
    (LIVE_OUTPUT / "decision.json").write_text(json.dumps(decision, indent=2) + "\n")

    log("[STAGE 5/6]", f"Formatting message ({mode}).")
    message = format_message(run_date, decision, mode)
    (LIVE_OUTPUT / "telegram_message.md").write_text(message + "\n")

    log("[STAGE 6/6]", "Publishing or recording dry run result.")
