#!/usr/bin/env bash
# StockHive fallback daily runner.
# Loads env and executes the direct fallback live runner.

set -euo pipefail

FALLBACK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_DIR="$(cd "${FALLBACK_DIR}/.." && pwd)"
cd "$REPO_DIR"
...
LOG_DIR="${FALLBACK_DIR}/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/run-${RUN_DATE}.log"

echo "[stockhive-fallback] run=${RUN_DATE} repo=${REPO_DIR}" | tee -a "$LOG_FILE"
...
STOCKHIVE_PUBLISH_MODE="${STOCKHIVE_PUBLISH_MODE:-live}" \
  .venv/bin/python fallback/scripts/run_live_option_b.py \
  2>&1 | tee -a "$LOG_FILE"

echo "[stockhive-fallback] done run=${RUN_DATE}" | tee -a "$LOG_FILE"
